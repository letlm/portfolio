export const projects = [
  {
    id: 1,
    title: "KenzieHub",
    img: "https://i.imgur.com/BMOqmxH.png",
    demo: "https://kenzie-hub-letlm.vercel.app/",
    repo: "https://github.com/letlm/kenziehub-REACT",
    react: "https://i.imgur.com/kjwP6rK.png",
    description:
      "Aplicação usando API. O usuário pode criar uma conta, fazer login e cadastrar tecnologias que estudou/está estudando, além de ser possível editar as tecnologias a partir do nível de estudos e excluí-las.",
  },
  {
    id: 2,
    title: "NuKenzie",
    img: "https://i.imgur.com/Rkxulnx.png",
    demo: "https://nu-kenzie-letlm.vercel.app/",
    repo: "https://github.com/letlm/nukenzie-REACT",
    react: "https://i.imgur.com/kjwP6rK.png",
    description:
      "Aplicação de gerenciamento de gastos na qual é possível cadastrar entradas e despesas, além de possuir a funcionalidade de filtrar os cadastros bem como excluí-los.",
  },
  {
    id: 3,
    title: "BurguerKenzie",
    img: "https://i.imgur.com/5vrQjVV.png",
    demo: "https://kenzie-burguer-letlm.vercel.app/",
    repo: "https://github.com/letlm/kenzieBurguer-REACT",
    react: "https://i.imgur.com/kjwP6rK.png",
    description:
      "Aplicação que simula uma hamburgueria, podendo adicionar apenas uma unicade de cada produto. Além disso, ainda apresenta o valor total do carrinho, sendo possível remover os produtos do carrinho.",
  },
  {
    id: 4,
    title: "KenzieShop - Context Api",
    img: "https://i.imgur.com/z9xMyqt.png",
    demo: "https://kenzieshop-contextapi-letlm.vercel.app/",
    repo: "https://github.com/letlm/kenzieshop-contextAPI",
    react: "https://i.imgur.com/kjwP6rK.png",
    description:
      "Simulação de um carrinho de ecommerce feita com context Api. O usuário consegue adicionar produtos no carrinho e removê-los, além de simular uma compra.",
  },
  {
    id: 5,
    title: "KenzieShop - Redux",
    img: "https://i.imgur.com/kUlUnOa.png",
    demo: "https://kenzie-shop-redux-letlm.vercel.app/",
    repo: "https://github.com/letlm/kenzieshop-REDUX",
    react: "https://i.imgur.com/kjwP6rK.png",
    description:
      "Simulação de um carrinho de ecommerce feita com redux, utilizando o redux-thunk para que os produtos adicionados permaneçam no carrinho e sejam removidos apenas pelo usuário.",
  },
  {
    id: 6,
    title: "BeraTop - Em grupo",
    img: "https://www.google.com.br/",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    react: "https://i.imgur.com/kjwP6rK.png",
    description: "",
  },
  {
    id: 7,
    title: "Portfólio",
    img: "https://i.imgur.com/xbbdnOR.png",
    demo: "https://portfolio-letlm.vercel.app/",
    repo: "https://github.com/letlm/portfolio",
    react: "https://i.imgur.com/kjwP6rK.png",
    description: "Meu portfólio criado com React JS",
  },
  {
    id: 8,
    title: "Formulário de Cadastro",
    img: "https://i.imgur.com/92h3EkY.png",
    demo: "https://react-entrega-s2-formulario-de-cadastro-letlm.vercel.app/",
    repo: "https://github.com/letlm/portfolio",
    react: "https://i.imgur.com/kjwP6rK.png",
    description:
      "Aplicação que simula um formulário de cadastro utilizando as bibliotecas: React Hook Form, Yup e React Router Dom.",
  },
  {
    id: 9,
    title: "Torre de Hanoi",
    img: "https://i.imgur.com/T3tgUK3.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "O objetivo consistia em reproduzir o quebra-cabeças 'Torre de Hanoi', no qual os discor e torres deveriam ser criados com o DOM",
  },
  {
    id: 10,
    title: "Selector Hat",
    img: "https://i.imgur.com/jfyPE6q.png",
    demo: "https://selector-hat-letlm.vercel.app/",
    repo: "https://www.google.com.br/",
    html: "https://i.imgur.com/NX3u7e4.png",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "Inspirado na 'Magic 8-ball' mas com uma pegada no universo Harry Potter. Foi realizado com Programação Orientada a Objetos(POO). O usuário escreve uma pergunta e o Selector Hat responderá!",
  },
  {
    id: 11,
    title: "Blog M2",
    img: "https://i.imgur.com/wj4HHlo.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "Fazendo o consumo de uma API, é possível fazer o cadastro e login do usuário. Depois de logado é possível fazer posts no blog, excluir ou editar os próprios posts, bem como ler os posts de outros usuários.",
  },
  {
    id: 11,
    title: "Quadro de Medalhas",
    img: "https://i.imgur.com/D9yd8cq.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "Quadro que exibe a quantidade de medalhas dos 15 países que mais tiveram vitórias nas competições das Olimpíadas. Para obter os dados foi utilizada uma api e para contruir a tabela o DOM.",
  },
  {
    id: 11,
    title: "Lista de Produtos",
    img: "https://i.imgur.com/WIKnQ0T.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "Aplicação simulando um 'supermercado', sendo possível adicionar os produtos no carrinho, bem como é possível fazer a busca por nome ou categoria.",
  },
  {
    id: 12,
    title: "Contador de Frequência",
    img: "https://i.imgur.com/UF2mhax.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "A aplicação registra a frequência de cada uma das letras e palavras usadas em um determinado texto/palavra que o usuário digitar.",
  },
  {
    id: 13,
    title: "Rolando Dados",
    img: "https://i.imgur.com/pW1iwDg.png",
    demo: "https://fantastic-garbanzo-5ac6368a.pages.github.io/",
    repo: "https://www.google.com.br/",
    js: "https://i.imgur.com/TLXXHeQ.png",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "A aplicação simula a rolagem de dois dados várias vezes, no qual é registrado a frequência de cada resultado e ainda é encontrado um gráfico com as frequências obtidas.",
  },
  {
    id: 14,
    title: "KenzieFlix",
    img: "https://i.imgur.com/4qbofIP.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "O objetivo era a estilização de um layout de uma plataforma de streaming utilizando CSS com conceitos de flex-box.",
  },
  {
    id: 15,
    title: "Menu Restaurante",
    img: "https://i.imgur.com/nOtBS6T.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "O objetivo era apenas a estilização de um Menu de Restaurante de duas formas diferentes utilizando CSS",
  },
  {
    id: 16,
    title: "Currículo Guru",
    img: "https://i.imgur.com/jFvFlqG.png",
    demo: "https://www.google.com.br/",
    repo: "https://www.google.com.br/",
    css: "https://i.imgur.com/RF1a25i.png",
    description:
      "O objetivo era apenas a estilização de um currículo Guru utilizando CSS.",
  },
];

//reactjs: https://i.imgur.com/kjwP6rK.png
//html: https://i.imgur.com/NX3u7e4.png
//js: https://i.imgur.com/TLXXHeQ.png
//css: https://i.imgur.com/RF1a25i.png
